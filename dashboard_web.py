...
def filtrar_dados(meses, grupos):
...
def render_tab(tab, meses, grupos):
    rec, desp = filtrar_dados(meses, grupos)
...
def build_page_executiva(rec, desp, meses_filtro):
    ...
    # --- Donut Receita por Grupo ---
    fig_donut = go.Figure(go.Pie(
        labels=rec_grp["DescGrupo"], values=rec_grp["Receita"],
        hole=0.55, textinfo="label+percent",
        marker=dict(colors=["#1B5E20", "#2E7D32", "#43A047", "#66BB6A", "#A5D6A7"]),
        textfont=dict(size=10),
    ))
    fig_donut.update_layout(**chart_layout("Composicao das Receitas", 320))
